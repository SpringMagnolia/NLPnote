...
$$
tfidf_i = tf*idf = \frac{词i的数量}{总词语数}*log\frac{总文档数}{包含词i的文档数}
$$
其中tf称为文档频率，idf为逆文档频率
...
把$1-b+b\frac{d}{avdl}$中的b看成0，那么此时中间项的结果为$\frac{(k+1)tf}{k+tf}$，取值从1到正无穷。当tfidf的非常大的时候，通过调节k的值，可以把它限制到一定的范围，`k往往取值1.2`。其变化如下图
...
可以看到，当句子的长度约端，$1-b+b\frac{|d|}{avdl}$的值是越小，作为分母的位置，会让整个第二项越大，从而达到提高短文本句子的BM25的值的效果。当b的值为0，可以禁用归一化，`b往往取值0.75`
